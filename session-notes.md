...
### 2026-06-29 session (part 2) — interior-page design overhaul (on `staging`, w/ Brees)
Brees feedback pass. Done:
- **Header / Client Corner trimmed (all 8 pages, desktop + mobile):** removed Contact, Call, Text, Request an Appointment from the Client Corner dropdown. Now ONLY Payment Options + FAQs. (Left the mobile slide-out's separate bottom contact block — Call/Text/Request button — as that's the mobile menu footer, not "under Client Corner.")
...
**SURGERY FINAL (Brees-approved):** hero(video) → **overlap intro card** lifting into the hero (`.svc-intro-card`; hero title raised via `.service-hero .inner` padding) → Core **split** (numbered list + image) → **sticky-parallax** pull-quote "From check-in to pick-up." (`.svc-quote`: 150vh, `background-attachment:fixed` + `position:sticky` wrap, CSS-only so it shows with Reduce Motion ON; mobile falls back) → Advanced **dark band** → CTA image band. Intro lead = editorial eyebrow+lead w/ italic `.light` accent, single column aligned to the 1440 frame (NOT the 1180 svc-wrap — that mismatch read as off-center). Generous space below the card (`.svc-intro` padding-bottom clamp(96,14vh,180)). **Reveal-on-load motion** added to shared `site.js` (reduced-motion-gated IntersectionObserver; hero animates in, sections rise on scroll + stagger).
...
- **End-of-life (REDESIGNED, then refined per Brees feedback):** overlap card → **Hospice** = warm **split + image** (careers.jpg) → **Euthanasia + In-home** = **side-by-side 50/50 dark twin band** (`.eol-twin`/`.eol-twin-col`, vertical hairline divider, ink-night) → **closer** "Here for you" = **image band** (hero-bmah-poster beach + soft scrim, white CTA). Earlier iterations (immersive parallax band for euth + quiet-poetic in-home) were REPLACED by the twin band per feedback; plain centered `.eol-section` blocks were already dropped.

**Brees feedback pass (2026-06-29):** (1) **Wellness sec2** plain lead → overlap intro card. (2) **Diagnostics imaging** de-boxed-on-cream → **dark immersive band** (`.dx-section.dx-dark`). (3) **EOL closer** got an image. (4) **EOL euth+in-home** → 50/50 twin. (ad5d8c4)

**Feedback v2 (a4bafe2):** (a) EOL **twin spacing fixed** — `.eol-twin-grid` reframed to the page wrap (max-width 1440, padding clamp(40,6vw,86)); col1 left now = frame-left 86, col2 right = frame-right 1354, centered divider at 720, body text fills each column (was off-frame left + dead right gap). (b) Diagnostics X-ray reasons: **glass pills KILLED** → clean dashed 2-col list (`.dx-dark .dx-tags` grid + sea dash markers). (c) **One-side (left-rule) border card style removed everywhere** — Brees dislikes it: `.svc-note` (wellness titers), `.dn-tip-card` (dentistry "between visits"), `.dx-subcard` (diagnostics ultrasound) all → editorial **top hairline** (border-top 1px) instead.
**Brees DISLIKES (locked rules):** glass/translucent pills & boxes; one-side accent borders (left-rule cards). Prefer: editorial top-hairline dividers, dashed/numbered hairline lists, clean fills.
**Feedback v3 (dedff88 + 74a8d9b):** (a) **Contact** added as a top-level main nav item (desktop + mobile) on all pages — was pulled from Client Corner dropdown but never re-added. (b) contact.html: removed Contact still lingering in its desktop Client Corner dropdown + added video hero (was image-only). (c) Wellness `.svc-list` checkmarks → **dash markers** (match X-ray reasons). (d) Dentistry "Our approach" journey: per-step **reveal stagger** (each beat reveals on scroll; gated behind prefers-reduced-motion — Brees runs Reduce Motion ON so he must toggle it off to preview). (e) **Diagnostics imaging rebuilt to wellness-sec3 layout** — `.svc-item` numbered list (thumbnail + num + body) on dark `.dx-imaging` section; X-ray reasons = dashed `.svc-list`, ultrasound subtypes = top-hairline blocks.
**STILL placeholder:** wellness service-block thumbnails + NEW diagnostics imaging thumbnails (`placehold.co`). Team.html + payment-options.html still 404 (no copy).
All copy verbatim. All validated: 0 overflow @375/768/1440, valid heading order, screenshots confirmed.

**⏭ STILL TODO:**
- Real assets: per-service hero VIDEOS (only 2 placeholder clips, reused), and real photos for splits/parallax bgs/thumbnails (lots of placeholder reuse of dog-cat-sleeping/careers/brick — flagged). Ask Brees re: licensing.
- Build placeholder pages still linked in nav: **team.html**, **payment-options.html** (currently 404).
- Confirm Instagram URL + main phone (still `000-000-0000`).
- Replicate the boxless + image treatment to **diagnostics** (dx-card grid → svc-items; it's a list of tests, maps cleanly) and **dentistry** (more bespoke: stat band + journey steps + panels — add images into its editorial sections rather than force svc-items). end-of-life is already boxless/editorial.
- Swap placeholder hero videos for real per-service footage; swap placehold.co thumbnails for real photos.
- Confirm end-of-life closer treatment (image band vs current gradient).

### 2026-06-29 session (part 1) — homepage sync + service-page polish (on `staging`, w/ Brees)
