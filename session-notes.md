...
**Brees feedback pass (2026-06-29, latest):** (1) **Wellness sec2** plain lead → overlap intro card (was the only service page missing it). (2) **Diagnostics imaging** de-boxed-on-cream → **dark immersive band** (`.dx-section.dx-dark` overrides; he wasn't a fan of the flat cream version). (3) **EOL closer** got an image. (4) **EOL euth+in-home** → 50/50 twin. All pushed (ad5d8c4).
