...
**⏭ STILL TODO (await Brees confirm on surgery as the model):**
- Give **dentistry / diagnostics / end-of-life** their OWN bespoke riffs too (each visually distinct — reuse/remix the new `.svc-quote`/`.svc-split`/`.svc-darkband` kit + their existing unique sections, but no two pages identical). Surgery is the proof-of-concept for the "riff, don't clone" approach.
